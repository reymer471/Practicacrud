...
public class CarritoControlador {
    ...
    public static void listarProductos(Context ctx) {
        ...
        modelo.put(
                "productos",
                ProductoServices.getInstancia().findAll()
        );

        List<ItemCarrito> carrito = obtenerCarrito(ctx);

        int totalArticulos = carrito.stream()
        ...
        // CAMBIO AQUÍ
        ctx.render("tienda", modelo);
    }

    public static void agregarAlCarrito(Context ctx) {
        Long idProducto =
                Long.parseLong(ctx.formParam("idProducto"));

        int cantidad =
                Integer.parseInt(ctx.formParam("cantidad"));

        Producto producto =
                ProductoServices.getInstancia()
                        .find(idProducto);
    ...
    public static void procesarCompra(Context ctx) {
        ...
        if (nombreCliente != null
            ...
            VentasProductos nuevaVenta =
                    new VentasProductos(
                            nombreCliente,
                            productosComprados
                    );

            VentasServices.getInstancia()
                    .crear(nuevaVenta);

            ctx.sessionAttribute(
                    "carrito",
                    new ArrayList<ItemCarrito>()
            );
        }
